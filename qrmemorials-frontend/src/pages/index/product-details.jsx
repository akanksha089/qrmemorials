...
export default function ProductDetails() {
    ...
    useEffect(() => {
        const fetchPackageDetails = async () => {
            try {
                const response = await fetch(`${API_BASE_URL}/api/v1/api-packages/${id}`, {
                    headers: {
                ...
                setPackageDetails(result.data); // because it's a single object
            } catch (err) {
    ...
    const addToCartApi = async (package_id, quantity = 1) => {
    ...
    const addToCart = async (packageId) => {
        try {
            ...
            const res = await addToCartApi(packageId, quantity); // correct usage
    ...
    return (
        <>
            ...
            <div className="bg-[#F8F5F0] dark:bg-dark-secondary py-5 md:py-[30px]">
                <div className="container-fluid">
                    <ul className="flex items-center gap-[10px] text-base md:text-lg leading-none font-normal text-title dark:text-white max-w-[1720px] mx-auto flex-wrap">
                        ...
                        <li>/</li>
                        <li className="text-primary">{packageDetails?.title ? packageDetails?.title : 'Classic Relaxable Chair'}</li>
                    </ul>
                </div>
            </div>
                        ...
                        <div className="w-full lg:w-[58%]">
                            <div className="relative product-dtls-wrapper">
                                ...
                                <div className="product-dtls-slider ">
                                    <div className={activeImage === 1 ? '' : 'hidden'}><img src={packageDetails?.image_url ? packageDetails?.image_url : product1} className='w-full' alt="product" /></div>
                                    <div className={activeImage === 2 ? '' : 'hidden'}><img src={packageDetails?.image_url ? packageDetails?.image_url : product2} alt="product" /></div>
                                    <div className={activeImage === 3 ? '' : 'hidden'}><img src={packageDetails?.image_url ? packageDetails?.image_url : product3} alt="product" /></div>
                                    <div className={activeImage === 4 ? '' : 'hidden'}><img src={packageDetails?.image_url ? packageDetails?.image_url : product4} alt="product" /></div>
                                </div>
                                <div className="product-dtls-nav">
                                    <div onClick={() => setActiveImage(1)} className='mb-2'><img src={packageDetails?.image_url ? packageDetails?.image_url : product1} alt="product" /></div>
                                    <div onClick={() => setActiveImage(2)} className='mb-2'><img src={packageDetails?.image_url ? packageDetails?.image_url : product2} alt="product" /></div>
                                    <div onClick={() => setActiveImage(3)} className='mb-2'><img src={packageDetails?.image_url ? packageDetails?.image_url : product3} alt="product" /></div>
                                    <div onClick={() => setActiveImage(4)}><img src={packageDetails?.image_url ? packageDetails?.image_url : product4} alt="product" /></div>
                                </div>
                            </div>
                        </div>
                        <div className="lg:max-w-[635px] w-full">
                            <div className="pb-4 sm:pb-6 border-b border-bdr-clr dark:border-bdr-clr-drk">
                                <h2 className="font-semibold leading-none">{packageDetails?.title ? packageDetails?.title : 'Classic Relaxable Chair'}</h2>
                                <div className="flex gap-4 items-center mt-[15px]">
                                    ...
                                    <span className="text-2xl sm:text-3xl text-primary leading-none block">${packageDetails?.price ? packageDetails.price : "89.00"}</span>
                                </div>
                                ...
                                {
                                    packageDetails && packageDetails.features && packageDetails.features.length > 0 ? (
                                        packageDetails.features && packageDetails.features.map((item, index) => (
                                            <div className='mt-5 md:mt-7 space-y-2'>
                                                <h4 key={index} className="text-xl md:text-[22px] font-semibold !leading-none">{item?.title}</h4>
                            ...
                            <div className="py-4 sm:py-6 border-b border-bdr-clr dark:border-bdr-clr-drk" data-aos="fade-up" data-aos-delay="200">
                                ...
                                <div className="flex gap-4 mt-4 sm:mt-6">
                                    {/* <button onClick={() => addToCart(packageDetails, quantity)} className="btn btn-solid" data-text="Add to Cart">
                                        <span>Add to Cart</span>
                                    </button> */}
                                    <button onClick={() => addToCart(packageDetails?.id, quantity)} className="btn btn-solid" data-text="Add to Cart"
                                        disabled={isAddingToCart}>
                                        <span>  {isAddingToCart ? 'Adding...' : 'Add to Cart'}</span>
                                    </button>
                                    <button onClick={() => addToWishlist(packageDetails, quantity)} className="btn btn-outline" data-text="Add to Wishlist">
            ...
            <div className="s-py-50">
                <div className="container-fluid">
                    <DetailTab packageDetails={packageDetails} />
                </div>
            </div>
