            ...
            <button className="relative hdr_wishList_btn" onClick={() => setWishList(!wishList)}>
                {
                    totalWishlistQuantity > 0 && (
                        <span className="absolute w-[22px] h-[22px] bg-secondary -top-[10px] -right-[11px] rounded-full flex items-center justify-center text-xs leading-none text-white">{totalWishlistQuantity}</span>
                    )
                }
                                ...
                                wishlist.map((item, index) => (
                                    ...
                                    >
                                        ...
                                        <div
                                            ...
                                            className="wishList_item_close absolute top-0 right-0 w-6 h-6 flex items-center justify-center bg-title dark:bg-white bg-opacity-10 dark:bg-opacity-10 group duration-300 hover:bg-primary dark:hover:bg-primary"
                                        >
            ...
            <div className={`hdr_cart_popup w-80 md:w-96 absolute z-50 top-full right-0 sm:right-10 xl:right-0 bg-white dark:bg-title p-5 md:p-[30px] border border-primary ${cart ? '' : 'hidden'}`}>
                ...
                <div>
                                ...
                                return (
                                    <div className="flex gap-[15px] relative pb-[15px] mb-[15px] border-b border-bdr-clr dark:border-bdr-clr-drk group" key={index}>
                                        <Link to={`/product-details/${item.id}`} className="block">
                                            <img className="w-[70px] md:w-auto h-full" src={item.image_url} alt="cart" />
                                        </Link>
                                        <div>
                                            <div className="flex items-center gap-2">
                                                {/* <span className="text-[14px] md:text-[15px] leading-none block">{item.title}</span> */}
                                                <span className="w-[6px] h-[6px] rounded-full bg-primary"></span>
                                            ...
                                            <h6 className="text-base md:text-lg font-semibold !leading-none mt-[10px] mb-4">
                                                <Link to={`/product-details/${item.id}`}>{item.package_name}</Link>
                                            </h6>
                                            <IncreDre quantity={item.quantity}
                                                onQuantityChange={(qty) => updateQuantity(index, qty)} />
                                        </div>
                                        <div onClick={() => handleRemoveItem(item?.id)} className="wishList_item_close absolute top-0 right-0 w-6 h-6 flex items-center justify-center bg-title dark:bg-white bg-opacity-10 dark:bg-opacity-10 group hover:bg-primary dark:hover:bg-primary opacity-0 group-hover:opacity-100 text-title dark:text-white duration-300 hover:text-white">
                                            <LuX className="text-title dark:text-white duration-300 group-hover:text-white" />
                                        </div>
                                    </div>
                                )
                    ...
                    <div className="pt-5 md:pt-[30px] mt-5 md:mt-[30px] border-t border-bdr-clr dark:border-bdr-clr-drk">
                        <h4 className="mb-5 md:mb-[30px] font-medium !leading-none text-lg md:text-xl text-right">Subtotal : ${subtotal.toFixed(2)}</h4>
